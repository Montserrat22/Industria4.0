 ...
 */
@Entity
@Table(name = "procesodos")
@NamedQueries({
    @NamedQuery(name = "Procesodos.findAll", query = "SELECT p FROM Procesodos p"),
    @NamedQuery(name = "Procesodos.findById", query = "SELECT p FROM Procesodos p WHERE p.id = :id"),
    @NamedQuery(name = "Procesodos.findByHora", query = "SELECT p FROM Procesodos p WHERE p.hora = :hora"),
    @NamedQuery(name = "Procesodos.findByFecha", query = "SELECT p FROM Procesodos p WHERE p.fecha = :fecha"),
    @NamedQuery(name = "Procesodos.findByStatusproceso", query = "SELECT p FROM Procesodos p WHERE p.statusproceso = :statusproceso")})
public class Procesodos implements Serializable {
    ...
    @OneToMany(mappedBy = "pdos")
    private Collection<Proceso> procesoCollection;

    public Procesodos() {
    }

    public Procesodos(Short id) {
        this.id = id;
    }
    ...
    public boolean equals(Object object) {
        ...
        if (!(object instanceof Procesodos)) {
            return false;
        }
        Procesodos other = (Procesodos) object;
    ...
    public String toString() {
        return "entidades.Procesodos[ id=" + id + " ]";
    }
